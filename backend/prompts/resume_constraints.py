...
- 技能分组不超过 4 组，同类技能合并为逗号分隔列表；篇幅不足时优先在 Skills 版块内部把各组压成单行（如「Languages: Python, SQL」），并去掉空话套话
- 教育经历为必要模块，禁止为凑页数删除；篇幅不足时优先：①精简 Skills/Awards 版块内部条目为单行并删冗余 ②compact 间距 ③缩短经历 bullet；不得删除教育经历；禁止把 Skills 与 Awards 两个版块合并成一个版块
- 优先保留与目标岗位最相关的 2-3 段核心经历；仅低相关实习/项目可合并或省略，不得删除教育经历
- 获奖在空间紧张时优先在 Awards 版块内部把每条压成单行（如「一等奖学金：2023」）并精简描述，可再删次要条目，但不得删除教育模块
...
精简策略（按优先级）：
1. 【最高优先】在 Skills 版块内部、Awards 版块内部分别处理（禁止把两个版块合成一个）：
   - 每条技能/奖项压成单行（title 并入 content，如「Languages: Python, SQL」）
   - 多行/项目符号列表改为逗号分隔；删除重复词与空话（熟练掌握/proficient in 等）
   - 能精简的描述尽量缩短，但保留关键技能名与奖项名
2. 缩短 summary，删除空泛形容词
3. 使用更紧凑的 spacing_scale（compact）与较小字号；教育经历等必要模块必须保留
4. 合并或删除低相关次要经历/项目——仅可删除用户已在优化对话中确认同意的 experiences_to_remove 条目；未确认的经历必须保留；禁止删除教育经历
5. 每条经历减少 bullet 数量，保留最强成果
