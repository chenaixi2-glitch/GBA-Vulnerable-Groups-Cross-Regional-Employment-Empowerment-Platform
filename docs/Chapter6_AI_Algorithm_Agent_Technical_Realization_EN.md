...
Corporate accounts receive **free baseline recruitment tooling**: job posting and editing, embedded My Jobs management, and company profile maintenance, plus ungated portal helper cards (candidate-screening / talent-matching UI shells, workplace audit, certification). The generative Career Copilot itself—smart resume, interactive mock interview, and learning path—ships on the **individual** portal (`individual/demo-*.html`), not as deep-linked corporate clones. **Premium** on the corporate side—primarily the gated HR Interview Kits modal and performance-analytics views—requires `has_premium_access` after a donation-box contribution. Vulnerable individuals receive free full access; non-vulnerable individuals unlock platform features by any donation amount. Permissioning is enforced in `access.service.js` and front-end guards (`data-require-access`, `data-require-premium-access`), not by swapping model weights. This design turns feature tiers into levers of a social-enterprise business model (expanded in Section 6.5).

From a requirements-engineering view, the corporate split also prevents a common failure of social platforms: giving employers unlimited free advanced hiring aids that extract value while leaving legal-aid underfunded. By tying Premium HR kits and analytics to donation, the requirement set encodes reciprocity into the product backlog rather than treating ethics as a separate PDF.
...
| Class | Scope |
|-------|--------|
| **A. Fully launched** | LangGraph Career Copilot (planner + JD / profile / gap / content / render / interview / question / answer-evaluation / learning-path agents); resume SSE generation, polish, Hunyuan translation, WeasyPrint/DOCX export; interactive mock interview via REST (debrief + persistence; shares `CopilotState`, outside the graph); learning-path timeline edit/save; Redis session + MySQL archives; Node skill-first match scoring + inclusive hard filters; donation/Premium access (corporate HR kits & analytics gated; individual Copilot on `individual/`) |
...
[FastAPI Career Copilot]
        ...
        ├─ Interview REST: interactive start/turn/poll/end/save  (graph-external; shared state)
...
**Programme layering.** `interview_program.py` defines three versions with turn caps. **Quick** (~25–35 min, default): a merged screening/final stage plus a full professional/technical round—short enough for silver-generation users, but still covering role depth. **Full** (~50–60 min): screening → professional/technical → final as three separate stages. **Specialized** (~15–30 min): a single deep-dive track (technical, final negotiation, or resume deep-dive). Question banks always include a self-introduction item and add role-informed categories using keyword track hints.

**Interaction modes.** Question-bank generation and answer evaluation can run through the LangGraph `interview_agent` / `answer_evaluation_agent` nodes (via `/api/chat`). Custom mode accepts employer-specific questions and returns reference answers. **Interactive mock** is deliberately graph-external: it pre-builds a bank queue, accepts turns through `/interactive/start|turn|poll|end`, and produces asynchronous feedback plus optional follow-ups, so multi-turn polling is not forced through planner routing. Judge-model scores cover relevance, groundedness, and actionability. Debrief aggregates strengths, improvements, and next drills. Persistence endpoints store banks and full sessions for logged-in reload.
...
**Resources and timeline.** Analysis prompts recommend lightweight courses, drills, or projects with hour estimates. When the user supplies daily hours and a timeline intent, the system computes span with transparent two-step arithmetic—`total_days = ceil(total_hours / daily_hours)`, then `total_weeks = ceil(total_days / 7)`—and asks the LLM to allocate phases. Front-end editors mutate phase fields; `PUT /timeline` and `POST /save` persist plans with history reload. Recommendations favour short, practical units aligned with flexible roles (customer service, remote operations, annotation-style work) rather than multi-year credentials.
...
Corporates do **not** receive a second LangGraph deployment. Free baseline covers posting inclusive jobs, managing My Jobs, editing company profiles, and opening ungated portal HR helper cards (screening / talent shells, audit, certification). The launched generative Career Copilot remains on the individual portal. Premium anchors use `data-require-premium-access` on the corporate Interview Kits modal and analytics views; `platform-access.js` intercepts and routes undonated corporates to the donation page until `DonationModel` counts unlock `has_premium_access`. Differentiation is therefore an **access-and-surface** design: shared entitlement policy in Node, employer-facing portal UX, candidate-facing Copilot elsewhere—not duplicated prompt libraries or a separate model cluster.

**Commercial interpretation linked to Chapter 4.** In an ECOM7001 social-commerce framing, the donation box is the sustainability hinge. Non-vulnerable individuals unlock the personal Copilot by donating; corporates keep basic recruitment free but pay (via donation) for Premium HR kits and analytics, contributing to a **legal-aid funding pool**. Premium features function as **incentive-compatible digital goods**: clear employer value, low marginal cost on shared infrastructure, and a measurable public-benefit externality. Access booleans in `access.service.js` embody that plan—every Premium unlock is simultaneously a donation event and an entitlement flip. Dashboard messaging and i18n copy explain the bargain; the system does not call an LLM to invent promotional slogans. Chapter 4 states why donations sustain the platform; Chapter 6 shows how feature gates enforce the bargain without fracturing the model stack.

**Employer-side value proposition.** Free job posting, skill-first matching, and inclusive hard filters let employers reach vulnerable applicants without buying a second AI stack. Premium Interview Kits aim at more consistent, structured screening question packs for hiring managers; analytics views summarise recruitment activity once donated. Candidates still prepare on the individual Copilot (resume → mock → learning path) with the same skill/gap vocabulary that matching consumes—reducing the asymmetry where employers use opaque ATS filters applicants never see. The donation requirement for Premium publicly signals that advanced employer tooling is contingent on contributing to legal-aid capacity in the same ecosystem.

**Implementation footprint.** One FastAPI Copilot cost centre for individuals; one Express access policy for both roles; corporate portal cards open local guided modals rather than deep-linking to `individual/demo-*.html` in the current cut. That scope is deliberate Class A honesty: commercial reciprocity is enforced where it is wired, without over-claiming that every HR card already hosts the full candidate agent pipeline.
...
3. **Access justice.** Vulnerable users free; corporate Premium HR kits/analytics tied to legal-aid donations—technical enforcement of social-enterprise ethics. The candidate Copilot stays on the individual portal.  
...
| Corporate donation AI copywriter | Access policy + Premium HR kits/analytics gating + i18n entitlement messaging |
